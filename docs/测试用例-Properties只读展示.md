...
覆盖应用内 Properties 面板的默认展开、手动折叠、文档状态隔离、不同文档打开顺序、全类型只读展示、自动换行，以及 HTML 导出的一致行为。回归验证 front matter 解析、正文分离和相关前端功能不受影响。
...
### TC-015: 首个无 Front Matter 文档不阻止后续面板显示
- 前置条件：准备文档 A（无 front matter）和文档 B（含合法 YAML mapping front matter）。
- 操作步骤：启动应用后先打开文档 A，再打开文档 B。
- 预期结果：文档 A 不显示 Properties；文档 B 成为活动文档后立即显示默认展开的 Properties，属性内容完整可见。
- 优先级：P0

### TC-016: 面板随活动文档双向切换
- 前置条件：同时打开文档 A（无 front matter）和文档 B（含合法 front matter），文档 B 的 Properties 已手动收起。
- 操作步骤：从文档 B 切换到文档 A，再切回文档 B；随后展开面板，再重复一次来回切换。
- 预期结果：切到文档 A 时面板消失；每次切回文档 B 时面板重新显示，并保留文档 B 当前的收起或展开状态。
- 优先级：P0
